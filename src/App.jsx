...
export default function App() {
  ...
  const [forcedTool, setForcedTool] = useState(null); // 'search_memory' | 'change_theme_color' | 'get_weather' | null
  ...
  const callGemini = async (messages, forcedSystemPrompt) => {
      ...
      // Nếu có forced tool, chèn thêm instruction
  ...
  const handleSendMessage = async () => {
    ...
    // Xử lý Force Tool: Nếu user chọn tool, ta sẽ tạo một System Prompt tạm thời cực mạnh để ép bot dùng
    ...
    try {
      ...
      saveSessionToDb({ id: currentSessionId, title: newTitle, messages: finalMessages, provider: config.activeProvider });
        ...
        setIsLoading(false); 
        setToolStatus(null); 
        setForcedTool(null); // Reset forced tool sau khi gửi
    }
  };
